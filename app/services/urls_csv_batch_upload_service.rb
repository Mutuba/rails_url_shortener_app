...
#   def import_urls(urls_array, batch)
#     progress = lambda { |_, num_batches, current_batch_number, _|
#       progress_percentage = (current_batch_number * 100) / num_batches
#       Rails.logger.info "Batch #{batch.id}: #{progress_percentage}% completed"
#       ActionCable.server.broadcast("user_#{@current_user.id}", { batch_id: batch.id, content: progress_percentage })
#     }
#     Url.import urls_array, batch_size: 2, batch_progress: progress, returning: :long_url
#   end
